 ...
 * It requires that the point cloud and image files form one-to-one
 * correspondence. And the file name should the sequence number or the
 * timestamp. What's more it adds camera information manager to meet some needs.
 * In addition, it plays the customized type of point cloud
 */
struct Point {
...
int main(int argc, char *argv[]) {
  ...
  std::string img_file_path = "/media/ramlab/SolidDisk/vel2cam2021/seq/02/img/";
  ...
  while (ros::ok()) {
    ...
    pcl::PointCloud<Point>::Ptr cloud_raw(new pcl::PointCloud<Point>);
